...
object PriceCalculator {
  ...
  @inline def apply[F[_] : MonadError[?[_], Throwable]](
    dependencies: Dependencies[F], logger: Logger[F]): PriceCalculator[F] =
      new PriceCalculatorImpl(dependencies, logger)

  private final class PriceCalculatorImpl[F[_] : MonadError[?[_], Throwable]](
    dep: Dependencies[F],
    logger: Logger[F]) extends PriceCalculator[F] {
