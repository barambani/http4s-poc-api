...
sealed private[errors] trait ServiceErrorInstances {
    ...
    new MonadError[F, ServiceError] {
      ...
      def handleErrorWith[A](fa: F[A])(f: ServiceError => F[A]): F[A] =
        F.handleErrorWith(fa)(
          thr =>
            f(
              if (thr.show.startsWith("InvalidShippingCountry"))
                InvalidShippingCountry.apply(thr).asServiceError
